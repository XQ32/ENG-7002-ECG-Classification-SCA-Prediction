...
% Overview: Build a new "PVC→recovery (variable window)" dynamics feature
%           set and train an imbalance-friendly binary classifier
%           (Alive=1 / Dead=0) using the *_info.mat metadata generated by
%           predict_shhs1_all.m (no EDF reading).
%           Focus on post-PVC dynamic metrics such as RR/T/QTc recovery
%           time constants, half-lives, overshoot, oscillations, and the
%           number of instabilities. Output confusion matrices in English.
% Key constraints:
%   - Only read *_info.mat (containing: predPVCIndices, patientVital, fs,
%     recordNumSamples, rGlobalAll, isPVCBeat, qrs_dur_vec, r_amp_vec,
%     sqi_vec, t_amp_vec, tGlobalIndices).
%   - If required fields for feature computation are missing, print a
%     suggested list at the end so the user can add them when generating
%     *_info.mat.
% Tunable parameters:
%   processFirstN        : Process first N records (<=0 means all), default 750.
%   splitRatioTest       : Test split ratio (default 0.20).
%   randomSeed           : Random seed.
%   params.*             : Recovery window/baseline/decision parameters.
%   modelOptions.*       : Model/cost/threshold moving/CV threshold tuning.
% Outputs:
...
%   results/sca_classifier_v3.mat                 : Trained model and options.
% Notes:
%   - For more morphology-related metrics (e.g., exact QT end, ST reference
%     point, QRS on/off indices, P-wave features), save more beat-wise
%     indices/amplitudes in predict_shhs1_all.m.
% Changelog:
%   2025-09-04: v3 initial version. New feature system and training/eval
%               workflow (no EDF reading).
% ========================================================================
...
% Read SHHS summary demographics/follow-up info (optional)
...
if exist(covarCsv,'file')
    ...
    % Normalize nsrrid to string
        if any(strcmpi(Tcov.Properties.VariableNames, 'nsrrid'))
            ...
            % Fallback: try the first column as patient or id
            idStr = string(Tcov{:,1});
        end
...
% Only process the first N *_info.mat records (<=0 means all)
processFirstN = 0;
...
% Recovery/baseline/threshold parameters (tunable)
params = struct();
params.baselineSec        = 50;   % Baseline lookback window before PVC (sec)
params.baselineMinBeats   = 10;   % Minimum baseline sinus beats
params.maxObsSec          = 50;   % Max observation time after PVC (sec)
params.consecStableBeats  = 10;    % Consecutive beats required to consider "stable recovery"
params.rrTolFrac          = 0.08; % RR recovery threshold: |RR-μRR| <= max(rrTolFrac*μRR, rrTolSigma*σRR)
params.rrTolSigma         = 1.5;
params.hrt_ts_low_thr     = 0.0;  % HRT-TS low threshold (smaller is more abnormal)
params.minRR              = 0.30; % RR quality threshold (sec)
params.maxRR              = 2.50; % RR quality threshold (sec)
params.minPVCPerRecord    = 10;   % Quality gate: minimum PVC count per record
params.minSQIRatio        = 0.60; % Quality gate: ratio of non-PVC beats with SQI=true

% Model and class imbalance settings
modelOptions = struct();
modelOptions.Method                = 'LogitBoost'; % Default to recent best
...
modelOptions.enableCostMatrix      = true;   % Cost matrix (bias to reduce FN for Dead)
modelOptions.costFP                = 2;
modelOptions.costFN                = 6;
modelOptions.enableThresholdMoving = true;   % Post-training threshold tuning (optimize Dead F1)
% Initialize default threshold with recent best (will be overridden by CV search)
...
% Feature selection/filtering disabled: use all features for training

% Candidate methods and search grid for auto selection (when Method='auto')
...
modelOptions.searchGrid.NumLearningCycles    = [200, 300];       % Number of boosting/bagging cycles
modelOptions.searchGrid.LearnRate            = [0.03, 0.05, 0.10]; % Only for boosting
modelOptions.searchGrid.MaxNumSplits         = [40, 60, 100];
modelOptions.searchGrid.MinLeafSize          = [8, 12, 20];
modelOptions.searchGrid.NumVariablesToSample = {'sqrt'};         % Sub-feature sampling strategy
modelOptions.autoSelectVerbose               = false;            % Quiet by default: print less progress
...
if exist(blacklistFile,'file')
    try
        ...
        vals = replace(vals, '''','');
        vals = vals(vals ~= "");
    % Mixed pure numeric strings and names; extract numeric IDs
...
numSkippedQuality = 0; % count of quality-gate exclusions

for iFile = 1:numel(allPaths)
    infoPath = allPaths{iFile};
    [~, baseName, ~] = fileparts(infoPath);
    recBase = erase(baseName, '_info');
    % Blacklist filter: support both numeric ID like '200002' and name 'shhs1-200002'
    ...
    % Map PVC R samples to nearest beat indices
    ...
    % Record-level SQI-good ratio (over available non-PVC beats)
    try
        j_all = (2:numBeats).';
    ...
    % Quality gate: PVC count and SQI ratio
    ...
    % ========== Per-PVC variable-window recovery dynamics features ==========
    % For each PVC: baseline stats, recovery half-lives, time constants,
    % overshoot, oscillation, number of stalls, AUC, etc.
    recMetrics = struct();
    ...
    hr_peak_accel_bpm = nan(numPVC,1); % peak of (1/RR_post1 - 1/muRR)*60 within first 2 beats

    % Longest "late instability variance" after PVC (>5s segment dRR variability)
    late_var_rr = nan(numPVC,1);

    % PVC -> interval to next non-PVC beat (seconds)
    pvc_to_next_nonPVC_sec = nan(numPVC,1);

    % —— Additional "discriminative/complexity" PVC-level features ——
    early_mean_dev_rr = nan(numPVC,1);   % Early (0–5s) mean deviation
    late_mean_dev_rr  = nan(numPVC,1);   % Late (5–15s) mean deviation
    slope_rr_dev_0_5s = nan(numPVC,1);   % Linear slope 0–5s
    slope_rr_dev_5_15s= nan(numPVC,1);   % Linear slope 5–15s
    sampen_rr         = nan(numPVC,1);   % Sample entropy of RR dev (m=2, r=0.2*std)
    lzc_rr            = nan(numPVC,1);   % LZ complexity of sign(dRR) sequence
    poincare_ratio_pp = nan(numPVC,1);   % Post(<=10s) SD1/SD2 divided by Pre SD1/SD2
    dev_rr_q95        = nan(numPVC,1);   % 95th percentile of deviation
    tamp_rr_corr      = nan(numPVC,1);   % Correlation between dev_rr and dev_tamp

    qtc_over_frac     = nan(numPVC,1);   % Fraction of QTc > 10% deviation
    qtc_over_mag      = nan(numPVC,1);   % Max excess magnitude of QTc (>10%)

    for kk = 1:numPVC
        pvcSample = pvcRidx(kk);
        idxPVC = idxPVC_all_sorted(kk);

    % Observation window end (next PVC or upper bound seconds)
        nextPVCsample = inf;
        if kk < numPVC
        ...
        % --- Baseline RR/T/QTc before PVC (non-PVC & good SQI) ---
        ...
        % Coupling ratio and HR peak acceleration (within first 2 beats)
        if isfinite(rr_pre)
        ...
        % Valid post-PVC RR/T/QTc sequences (non-PVC & good SQI & within window)
        ...
        % Recovery half-lives (50%/30%/10%), require params.consecStableBeats consecutive hits
        ...
        % Time constant (linear fit on ln(dev), robust fit using first K points)
        tau_rr(kk) = local_time_constant(dev_rr, t_after, 8);

        % AUC: ∫ dev_rr dt (time normalized by baselineSec to reduce long-record bias)
        auc_rr(kk) = local_auc(dev_rr, t_after, params.baselineSec);

        % Overshoot and oscillation/instability
        overshoot_rr(kk) = max(dev_rr) - min(dev_rr(1), dev_rr(1)); %#ok<NASGU>
        oscill_rr(kk)    = local_oscillation_index(dev_rr);
        stalls_rr(kk)    = local_recovery_stalls(dev_rr, 0.20, params.consecStableBeats); % Number of re-instabilities after first reaching 20% threshold
        ...
        % —— Compute additional "niche/complexity" features ——
        try
        ...
        try
            % Sample entropy (tolerant to short sequences)
            sampen_rr(kk) = local_sampen(dev_rr, 2, 0.2*std(dev_rr,'omitnan'));
        catch
            sampen_rr(kk) = NaN;
        end
        try
            % LZ complexity: sign of first-difference sequence
            d1 = diff(dev_rr(:));
        ...
        try
            % Poincaré SD1/SD2 ratio: Post<=10s divided by Pre baseline
        ...
        % Distance from PVC to next non-PVC beat (seconds)
        try
    ...
    safe_mean = @(v) agg(v, @mean); safe_med = @(v) agg(v, @median); safe_max = @(v) local_safe_max(v); safe_min = @(v) local_safe_min(v);

    % Compose record-level features
    R = struct();
    ...
    % Removed: auc_rr_mean, stalls_rr_mean, late_var_rr_median
    ...
    % Removed: auc_tamp_mean, QRS_Dur_PVC_mean
    % Baseline HRV RMSSD (integrated from PVC-specific baseline RR)
    try
    ...
    % Removed: sqi_good_ratio

    % —— New record-level aggregates (discriminative/niche) ——
    % Removed: rr_dev_early_mean_med, rr_dev_late_mean_med
    medEarly = safe_med(early_mean_dev_rr);
    medLate  = safe_med(late_mean_dev_rr);
    R.rr_dev_late_minus_early = medLate - medEarly;
    R.rr_dev_slope_0_5s_med   = safe_med(slope_rr_dev_0_5s);
    % Removed: rr_dev_slope_5_15s_med, rr_dev_sampen_med, rr_dev_lzc_med, poincare_ratio_postpre_med, rr_dev_q95_med, rr_dev_maxslope_0_5s_med
    R.tamp_rr_corr_med        = safe_med(tamp_rr_corr);
    R.qtc_overshoot_frac_mean = safe_mean(qtc_over_frac);
    % Removed: qtc_overshoot_mag_mean

    % Population instability across PVCs
    % Removed: coupling_ratio_cv, pvc_spacing_cv
    % Recovery rate (mean of rates corresponding to harmonic mean of half-lives)
    try
    ...
    % New: statistics of PVC-to-next-nonPVC distance (seconds)
    R.PVC_to_next_nonPVC_sec_mean = safe_mean(pvc_to_next_nonPVC_sec);
    R.PVC_to_next_nonPVC_sec_max  = safe_max(pvc_to_next_nonPVC_sec);
    R.PVC_to_next_nonPVC_sec_min  = safe_min(pvc_to_next_nonPVC_sec);
...
% Select numeric features (exclude record / patientVital)
...
% Exclude leakage or undesired predictors (e.g., censdate is follow-up length)
...
% Print all predictors used for training
try
    fprintf('\n=== Predictors (all numeric features, count=%d) ===\n', numel(predictorNames));
    disp(predictorNames.');
catch
end
...
% Group-stratified split by patient ID (avoid patient overlap across train/test)
...
for ii = 1:height(featureTable)
    ...
    % Fallback: extract digit substring
        tok2 = regexp(nm, '(\d+)', 'tokens', 'once');
        if ~isempty(tok2)
            patientId(ii) = string(tok2{1});
        else
            patientId(ii) = nm; % If parsing fails, use the full name
        end
    end
end
...
% ========== Normalization (Min-Max; fit on train, apply to test, clipped to [0,1]) ==========
try
    scaler = struct();
    ...
    fprintf('Applied min-max normalization to all predictors (train-fitted, clipped to [0,1]).\n');
catch ME
    fprintf('Normalization failed (skip): %s\n', ME.message);
end

% Print summary stats (mean, sd, missing ratio) for Train/Test
try
    fprintf('\n=== Summary stats (Train) ===\n');
    for i = 1:numel(predictorNames)
        fn = predictorNames{i};
        col = trainingFeatureTable.(fn);
        mu = mean(col, 'omitnan');
        sd = std(col, 'omitnan');
        miss = mean(isnan(col));
        fprintf('%-32s  mean=%10.6f  sd=%10.6f  miss=%.2f%%\n', fn, mu, sd, 100*miss);
    end
catch
end
try
...
% ===================== Univariate feature effectiveness =====================
...
% Print all features' univariate stats (full Train/Test)
try
    fprintf('\n-- Train: all features (sorted by AUC desc) --\n');
    TtrainAll = sortrows(featStatsTrain, 'AUC', 'descend');
    disp(TtrainAll);
catch
end
try
    fprintf('\n-- Test: all features (sorted by AUC desc) --\n');
    TtestAll = sortrows(featStatsTest, 'AUC', 'descend');
    disp(TtestAll);
catch
end
...
% ===================== No feature filtering/selection; use all numeric features =====================

% ===================== Train model (Ensemble + cost + threshold) =====================
...
% Threshold tuning (maximize F1 for Dead=0), saved to Options.pvcThreshold
...
% ===================== Evaluation (English confusion matrix) =====================
...
try
    % If Platt calibration is applied, local_predict_with_threshold has already
    % applied thresholding on the calibrated probabilities;
    % For metrics we should use calibrated probabilities rather than raw scores.
...
try
    ...
    % Reuse selected method and hyperparams from main model in ablation
        ...
        if isfield(modelOptions,'disableAutoInAblation') && modelOptions.disableAutoInAblation
            ...
            if isfield(abOptions,'selectedMethod')
                abOptions.Method = abOptions.selectedMethod; % Force selected method
            end
            ...
            abOptions.autoSelectVerbose = false; % Quiet
        end
...
function Trec = local_build_empty_record(recBase, patientVital, ~, ~)
    ...
    % New features (keep table structure consistent)
    R.rr_dev_late_minus_early = NaN;
    R.rr_dev_slope_0_5s_med   = NaN;
    R.tamp_rr_corr_med        = NaN;
    R.qtc_overshoot_frac_mean = NaN;
    R.recovery_rate_hmean     = NaN;
    % New: PVC-to-next-nonPVC distance (seconds)
    R.PVC_to_next_nonPVC_sec_mean = NaN;
    R.PVC_to_next_nonPVC_sec_max  = NaN;
    R.PVC_to_next_nonPVC_sec_min  = NaN;
...
function [muRR, sigRR, baseRR_vec, muTamp, muQTc] = local_baseline_stats(~, pvcSample, rGlobalAll, rr_between, isPVCBeat, sqi_vec, t_amp_vec, tGlobalIdx, qrs_dur_vec, fs, params)
    ...
    % Baseline T amplitude and approximate QTc
    ...
    try
    % Approximate QTc (consistent with main pipeline)
        j_ok = j_in;
...
function ts_val = local_approx_ts(idxPVC, pvcSample, obsEnd, rGlobalAll, rr_between, isPVCBeat, sqi_vec)
    % Approximate TS: among RR of the 5–15th non-PVC beats with good SQI after PVC,
    % take the maximum positive slope of any consecutive 5 RRs
    numBeats = numel(rGlobalAll);
...
function t50 = local_halflife(dev, t, thr, consecBeats)
    % Find the first time when there are consecBeats consecutive points with dev <= thr
    t50 = NaN;
...
function tau = local_time_constant(dev, t, K)
    % Fit ln(dev) over the first K points: dev(t) ~ exp(-t/tau)
    tau = NaN;
    if isempty(dev) || isempty(t), return; end
    dev = dev(:); t = t(:);
...
function A = local_auc(dev, t, normTime)
    % Trapezoidal integration ∫ dev dt, normalize time by normTime
    A = NaN;
    if numel(dev) < 2 || numel(t) < 2, A = 0; return; end
    dev = dev(:); t = t(:);
    dt = diff(t) ./ max(normTime, 1);
    if any(isnan(dev))
    % Forward-fill approximation for missing points
        for i = 2:numel(dev)
...
function cnt = local_recovery_stalls(dev, thr, consecBeats)
    % Number of re-instabilities after first reaching the threshold
    % (number of segments under thr minus 1)
    cnt = 0;
    if isempty(dev), return; end
    under = dev(:) <= thr;
    if ~any(under), return; end
    % Compress into run-length encoding
    transitions = [true; diff(under)~=0; true];
    % Count the number of entries into 'under'
    enterIdx = find([false; (~under(1:end-1) & under(2:end))]);
    % Count only 'under' runs whose length >= consecBeats
    i = 1; validSegments = 0;
...
function [trainedClassifier, validationAccuracy] = local_train_model(trainingTable, predictorNames, responseName, modelOptions)
    ...
    % If Method is not 'auto', train with the given method (no LearnRate for Bag)
    ...
    % =============== Auto method/param search (maximize F1[Dead]) ===============
    ...
    % Accurately compute total combinations (considering LearnRate for methods)
    totalComb = 0;
    ...
    trainedClassifier.RequiredVariables = predictorNames;
    % Record selected method and parameters
...
function model = local_threshold_tuning(model, Ttrain, predictorNames, responseName, modelOptions)
    % Perform K-fold CV on the training set to scan thresholds maximizing F1 of Dead(0)
    rng(0);
    try
    ...
    % Platt probability calibration (logistic) on CV scores
    calibBeta = [];
    try
...
% (removed) local_filter_low_discriminative

function [trainMask, testMask] = local_group_stratified_split(groupIds, y, testRatio, seed)
    % Group-stratified: ensure each group appears only in train or test
    rng(seed);
    groupIds = string(groupIds(:));
    y = double(y(:));
    ug = unique(groupIds);
    % Count class distribution per group
    ...
    % Greedy assignment to test set to match target class proportions
    ...
    for k = ord
        g = G(k);
    % If both under target, prefer group that moves both closer to targets
    ...
    % If still insufficient, randomly fill until close to target
    k = 1;
...
% (removed) local_filter_correlated_features
...
function s = local_sampen(x, m, r)
    ...
    % Compute ratio of template matches within distance r:
    % SampEn(m,r,n) = -ln( A_{m+1} / A_{m} )
    % Simplified, robust O(n^2) implementation for short/mid sequences
    count_m   = 0;
    count_m1  = 0;
    for i = 1:(n - m)
        Xi = x(i:(i+m-1));
...
function c = local_lzc_binary(b)
    % Lempel-Ziv complexity (binary sequence), normalized: c_norm = c * log2(n) / n
    c = NaN;
    b = b(:);
    b = b(isfinite(b));
    ...
    % Convert to char for classic LZ scanning
    s = char(b+'0').';
...
% (removed) local_select_features_mrmr
